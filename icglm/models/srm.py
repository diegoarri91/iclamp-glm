...
#         t_spikes = (t[arg_spikes[0]], arg_spikes[1], arg_spikes[2])
    ...
    def sum_convolution_kappa_t_spikes(self, t, mask_spikes, sd_stim=1):
        ...
        sum_conv_kappa_t_spikes = np.sum(sum_conv_kappa_t_spikes[::-1].reshape(len(t), -1), 1)
    ...
    def gh_log_likelihood_stim(self, stim, t, mask_spikes, sum_convolution_kappa_t_spikes, K, max_band, mu_stim=0,
                               sd_stim=1, stim_h=0, prior_noise=None, sd_noise=0, nbatch_noise=1):
        ...
        if not(sd_noise > 0):
            ...
            mask_spikes = np.repeat(mask_spikes[:, :, np.newaxis], nbatch_noise, axis=2)
            _, u, r = self.simulate_subthreshold(t, I, mask_spikes, stim_h=stim_h)
            log_likelihood = np.sum(u[mask_spikes]) / nbatch_noise - dt * v_noise_factor * np.sum(r) / nbatch_noise
            g_log_likelihood = dt * sum_convolution_kappa_t_spikes / nbatch_noise - \
                               dt * v_noise_factor * sd_stim / self.dv * self.kappa.correlate_continuous(t, np.sum(r.reshape(len(t), -1), 1)) / nbatch_noise
            t_support = self.kappa.support
            arg_support = int(t_support[1] / dt)
            h_log_likelihood = np.zeros((max_band, len(t)))
            for v in range(arg_support):
                h_log_likelihood[v, :] += -(dt * sd_stim / self.dv) ** 2 * v_noise_factor * K[v].correlate_continuous(t, np.sum(r.reshape(len(t), -1), 1)) / nbatch_noise
