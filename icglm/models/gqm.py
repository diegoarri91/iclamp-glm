...
class GQM(BayesianSpikingModel):
    ...
    def __init__(self, u0, kappa, eta, quad_kappa, quad_eta):
        self.u0 = u0
        self.kappa = kappa
        self.eta = eta
        self.quad_kappa = quad_kappa
        self.quad_eta = quad_eta
    ...
    def sample(self, t, stim, stim_h=0, full=False):
        ...
        while j < len(t):
            ...
            r[j, ...] = np.exp(
                kappa_conv[j, ...] + quad_kappa_conv[j, ...] - eta_conv[j, ...] - quad_eta_conv[j, ...] - u0)
            ...
            if np.any(mask_spk[j, ...]) and j < len(t) - 1:
                ...
                quad_eta_conv[j + 1:, mask_spk[j, ...]] += np.moveaxis(
                    np.diagonal(self.quad_eta.interpolate(t[j + 1:] - t[j + 1],
                ...
                for ii in range(len(idx[0])):
                    ...
                    for kk in arg_previous_spikes[0]:
                        ...
                        quad_eta_conv[aux2] += 2 * np.moveaxis(
                            np.diagonal(self.quad_eta.interpolate(t[j + 1:] - t[_k + 1],
                                                                  t[j + 1:] - t[j + 1])), -1, 0)  # [:, None]
            j += 1
    ...
    def simulate_subthreshold(self, t, stim, mask_spk, stim_h=0, full=False):
        ...
        if len(t_spikes[0]) > 0:
            ...
            # quad_eta_conv = self.quad_eta.convolve_discrete(t, t_spikes, shape=shape[1:])
            quad_eta_conv = self.quad_eta.convolve_continuous(t, spikes)
        else:
            eta_conv = np.zeros(shape)
            quad_eta_conv = np.zeros(shape)

        v = kappa_conv + quad_kappa_conv - eta_conv - quad_eta_conv - self.u0
        r = np.exp(v)

        if full:
            return kappa_conv, quad_kappa_conv, eta_conv, quad_eta_conv, v, r
        else:
            return v, r

    def use_prior_kernels(self):
        return self.kappa.prior is not None or self.eta.prior is not None or self.quad_kappa.prior is not None or \
               self.quad_eta.prior is not None
    ...
    def gh_log_prior_kernels(self, theta):
        ...
        if self.quad_eta.prior is not None:
            _log_prior, _g_log_prior, _h_log_prior = self.quad_eta.gh_log_prior(theta[1 + n_kappa + n_eta + n_quad_kappa:])
            log_prior += _log_prior
            g_log_prior[1 + n_kappa + n_eta + n_quad_kappa:] = _g_log_prior
            h_log_prior[1 + n_kappa + n_eta + n_quad_kappa:, 1 + n_kappa + n_eta + n_quad_kappa:] = _h_log_prior
    ...
    def get_theta(self):
        ...
        n_eta = self.eta.nbasis
        n_quad_kappa = self.quad_kappa.n_coefs
        n_quad_eta = self.quad_eta.n_coefs
        theta = np.zeros((1 + n_kappa + n_eta + n_quad_kappa + n_quad_eta))
        theta[0] = self.u0
        theta[1:1 + n_kappa] = self.kappa.coefs
        theta[1 + n_kappa:1 + n_kappa + n_eta] = self.eta.coefs
        theta[1 + n_kappa + n_eta:1 + n_kappa + n_eta + n_quad_kappa] = self.quad_kappa.coefs[np.triu_indices_from(self.quad_kappa.coefs)]
        theta[1 + n_kappa + n_eta + n_quad_kappa:] = self.quad_eta.coefs[np.triu_indices_from(self.quad_eta.coefs)]
        return theta
    ...
    def get_likelihood_kwargs(self, t, stim, mask_spikes, stim_h=0):
        ...
        n_quad_kappa = self.quad_kappa.n_coefs
        n_quad_eta = self.quad_eta.n_coefs
        X = np.zeros(mask_spikes.shape + (1 + n_kappa + n_eta + n_quad_kappa + n_quad_eta,))
        ...
        X_quad_eta = self.quad_eta.convolve_basis_continuous(t, spikes)
        ...
        X[:, :, 1 + n_kappa + n_eta + n_quad_kappa:] = -X_quad_eta

        X = X.reshape(-1, 1 + n_kappa + n_eta + n_quad_kappa + n_quad_eta)
    ...
    def set_params(self, theta):
        ...
        shape = (len(self.quad_kappa.tbins_x) - 1, len(self.quad_kappa.tbins_x) - 1)
        self.quad_kappa.coefs = np.zeros(shape)
        self.quad_kappa.coefs[np.triu_indices(self.quad_kappa.n)] = theta[1 + n_kappa + n_eta:1 + n_kappa + n_eta + n_quad_kappa]
        self.quad_kappa.coefs[np.tril_indices(self.quad_kappa.n)] = self.quad_kappa.coefs.T[np.tril_indices(self.quad_kappa.n)]

        self.quad_eta.coefs = np.zeros((self.quad_eta.n, self.quad_eta.n))
        self.quad_eta.coefs[np.triu_indices(self.quad_eta.n)] = theta[1 + n_kappa + n_eta + n_quad_kappa:]
        self.quad_eta.coefs[np.tril_indices(self.quad_eta.n)] = self.quad_eta.coefs.T[np.tril_indices(self.quad_eta.n)]
        return self
