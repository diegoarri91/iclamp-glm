...
class GLM(BayesianSpikingModel, BayesianDecoder):
    ...
    def __init__(self, u0, kappa, eta, nonlinearity=np.exp, nonlinearity_str='exp', nonlinearity_pars=None,
                 noise='poisson'):
        self.u0 = u0
        self.kappa = kappa
        self.eta = eta
    ...
    def sample(self, t, stim, stim_h=0, full=False, eta0=None):
        ...
        if self.kappa is not None:
            ...
            kappa_conv = np.zeros(stim.shape)

        j = 0
        while j < len(t):

            r[j, ...] = self.nonlinearity(kappa_conv[j, ...] - eta_conv[j, ...] - u0)
            # r[j, ...] = np.log(1 + np.exp(kappa_conv[j, ...] - eta_conv[j, ...] - u0))
            # r[j, ...] = np.max(np.concatenate((np.zeros(shape[1:])[None, :], (kappa_conv[j, ...] - eta_conv[j, ...] - u0)[None, :]), 1), 1)
            # r[j, ...] = np.exp((kappa_conv[j, ...] - eta_conv[j, ...] - u0) ** 3)
            # r[j, ...] = 0
    ...
    def use_prior_kernels(self):
        if self.kappa is not None and self.kappa.prior is not None:
            return True
        elif self.eta is not None and self.eta.prior is not None:
            return True
        else:
            return False
    ...
    def gh_log_prior_kernels(self, theta):
        ...
        if self.eta is not None and self.eta.prior is not None:
...
class GLMs:
    ...
    def sample(self, t, stim, mu=0, sd=1):

        from icglm.utils.time import get_dt
        dt = get_dt(t)

        if stim.ndim == 1:
            shape = (len(t), len(self.models))
        else:
            shape = (len(t), len(self.models)) + stim.shape[2:]

        eta_conv = np.zeros(shape)
        mask_spk = np.zeros(shape, dtype=bool)

        if stim.ndim == 1:
            u0 = np.array([glm.u0 for glm in self.models])
        else:
            u0 = np.array([glm.u0 for glm in self.models]).reshape((len(self.models),) + tuple([1] * (stim.ndim - 2)))

        if stim.ndim == 1:
            kappa_conv = np.stack([glm.kappa.convolve_continuous(t, stim * _sd + _mu)[:, 0] for glm, _mu, _sd in
                                   zip(self.models, mu, sd, np.arange(len(self.models)))], 1)
        else:
            kappa_conv = np.stack([glm.kappa.convolve_continuous(t, stim[:, ii] * _sd + _mu) for glm, _mu, _sd, ii in
                                   zip(self.models, mu, sd, np.arange(len(self.models)))], 1)

        j = 0
        while j < len(t):

            r = np.exp(kappa_conv[j, ...] - eta_conv[j, ...] - u0)
            ...
            for ii, model in enumerate(self.models):
                if np.any(mask_spk[j, ii]) and j < len(t) - 1:
                    eta_conv[j + 1:, ii, mask_spk[j, ii]] += model.eta.interpolate(t[j + 1:] - t[j + 1])[:, None]
